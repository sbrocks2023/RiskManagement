...
module.exports = cds.service.impl( async function(){
    ...
    const BPServ = await cds.connect.to("API_BUSINESS_PARTNER");
    this.on("READ", BusinessPartners, async(req)=>{
        req.query.where("LastName <> '' and FirstName <> ''");

        return await BPServ.transaction(req).send({
            query: req.query,
            headers: {
    ...
    this.on("READ", Risks, async (req, next) => {
       ...
       */
        ...
        const busienssPartners = await BPServ.transaction(req).send({
